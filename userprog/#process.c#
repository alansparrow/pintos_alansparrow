...
static bool load (const char *cmdline, void (**eip) (void), void **esp, char **saved_ptr);

/* This function is used to create new file descriptor for a new file */
...
/* This function is used to find the next available fd */
...
process_execute (const char *file_name) 
{
  ...
  /* Get file name without args*/
  char *saved_ptr;
  file_name = strtok_r((char *) file_name, " ", &saved_ptr);
...
start_process (void *file_name_)
{
  ...
  success = load (file_name, &if_.eip, &if_.esp, &saved_ptr);
  ...
  /* After finishing load, make parent run by using semaphore */
...
process_exit (void)
{
  ...
  /* Set exit value to true in case killed by kernel */
  if (thread_alive(cur->parent) &&
      cur->cp && cur->executable_file) {
    cur->cp->exit = true;
...
static bool setup_stack (void **esp, const char *file_name, char ** saved_ptr);
...
bool
load (const char *file_name, void (**eip) (void), void **esp, char **saved_ptr) 
{
  struct thread *t = thread_current ();
  ...
  /* Set up stack. */
  if (!setup_stack (esp, file_name, saved_ptr))
    goto done;
...
void remove_child_processes(void)
{
  ...
  struct list_elem *e = list_begin(&t->children_list);
...
int process_add_file(struct file *f)
{
  struct process_file *pf = 
    malloc(sizeof(struct process_file));
  ...
  thread_current()->fd++;
  list_push_back(&thread_current()->file_list,
		 &pf->elem);
  return pf->fd;
}
...
struct file *process_get_file(int fd)
{
  ...
  for (e = list_begin(&t->file_list);
       ...
       e = list_next(e)) {
    struct process_file *pf = 
      list_entry(e, struct process_file, elem);
    if (fd == pf->fd) {
      return pf->file;
    }
  }
